    ...
    if (userPlan === "free") {
      ...
      if (monthlyCount >= 150) {
        return res.status(403).json({
          error:
            "Monthly limit reached. Free plan includes 150 meetings per month. Please upgrade to Premium for unlimited meetings!",
          limitReached: true,
          monthlyCount,
          limit: 150,
        });
...
export const getSessionDetails = async (req, res) => {
  try {
    ...
    // Only host or meeting participant can view session details
    if (!m.host_id || m.host_id.toString() !== userId.toString()) {
...
export const getMeetingStats = async (req, res) => {
  try {
    ...
    const monthlyLimit = plan === "premium" ? null : 150;
