    ...
    <aside className="w-full sm:w-80 h-full bg-white border-l border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center text-slate-400 text-sm px-6">
            <div className="size-12 rounded-2xl bg-violet-50 border border-violet-100 flex items-center justify-center mb-3">
          ...
          messages.map((msg, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Message Bubble */}
                <div
                  className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs break-words ${
                    isMe
      ...
      >
        <input
          ...
          className="flex-1 min-w-0 bg-white border border-slate-200 focus:border-primary rounded-xl px-3.5 py-2.5 text-sm text-slate-900 placeholder-slate-400 outline-none transition-all shadow-xs"
        />

        <button
          type="submit"
          disabled={!text.trim()}
          className="shrink-0 p-2.5 rounded-xl bg-primary hover:bg-primary-hover text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer shadow-xs"
